...
def create_pocket_liner_arc(data, pocket, bulge, height): 
    ...
    NUM_POINTS = data["specs"]["TABLE_POCKET_LINERS_NUM_POINTS"]
...
def create_one_pocket_liner(data, pocket):
    ...
    """
    arcs = []
    arcs.append(create_pocket_liner_arc(data, pocket, 0.5*INCH, data["specs"]["TABLE_RAIL_HEIGHT"]))
...
def main():
    ...
    if WRITE_FILE:
        ...
        with open("liners.obj", "w") as file:
            for v in data["liners"]["vertices"]:
                file.write(f"v {v[0]} {v[1]} {v[2]}\n")
            for f in data["liners"]["faces"]:
                s = "f "
                for k in f:
                    s += f"{k+1} "
                file.write(s + "\n")
