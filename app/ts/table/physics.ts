...
type PhysicsLoopState = {
    internalTime: number | null;
    action: Collision | null;
}

class PhysicsLoop {
    public table: Table;
    public speed: number;
    public state: PhysicsLoopState;
     ...
     */
    public loadDiagram(diagram: any) {
        if (typeof(diagram) == "string") {
            try {
                return loadJSON(diagram).then((data: any) => {
                    if (!!data) {
                        this.table.resetBalls();
    ...
    public simulate(maxTime: number) {
